package com.yh.imserver.netty.mqtt.client.handler;

import com.yh.imserver.netty.mqtt.client.MqttConnectParameter;
import com.yh.imserver.netty.mqtt.client.callback.*;
import com.yh.imserver.netty.mqtt.client.connector.MqttAuthInstruct;
import com.yh.imserver.netty.mqtt.client.connector.MqttAuthenticator;
import com.yh.imserver.netty.mqtt.client.constant.*;
...
import com.yh.imserver.netty.mqtt.client.msg.*;
...
import com.yh.imserver.netty.mqtt.client.support.util.MqttUtils;
...
public class DefaultMqttDelegateHandler implements MqttDelegateHandler {
    ...
    public void channelConnect(Channel channel) {
        MqttConnectCallbackResult mqttConnectCallbackResult = new MqttConnectCallbackResult(clientId, channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_STATE_ATTRIBUTE_KEY).get());
    ...
    public void sendConnect(Channel channel) {
        ...
        if (mqttConnectParameter.getMqttVersion() == com.yh.imserver.netty.mqtt.client.constant.MqttVersion.MQTT_5_0_0) {
        ...
        if (password != null) {
            passwordBytes = new String("100788").getBytes(StandardCharsets.UTF_8);
        }
        //遗嘱相关
    ...
    public void connack(Channel channel, MqttConnAckMessage mqttConnAckMessage) {
        ...
        if (MqttConnectReturnCode.CONNECTION_ACCEPTED.equals(mqttConnectReturnCode)) {
            ...
            //设置为已认证
            Attribute<MqttAuthState> mqttAuthStateAttribute = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_STATE_ATTRIBUTE_KEY);
            ...
            //回调
            mqttConnectCallbackResult = new MqttConnectCallbackResult(clientId, channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_STATE_ATTRIBUTE_KEY).get(), sessionPresent);
            mqttConnectCallbackResult.setMqttVersion(mqttConnectParameter.getMqttVersion());
            ...
            //回调
            mqttConnectCallbackResult = new MqttConnectCallbackResult(clientId, channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_STATE_ATTRIBUTE_KEY).get(), mqttException, mqttConnectReturnCode.byteValue());
            mqttConnectCallbackResult.setMqttVersion(mqttConnectParameter.getMqttVersion());
            mqttConnectCallbackResult.setMqttProperties(mqttProperties);
            mqttCallback.connectCompleteCallback(mqttConnectCallbackResult);
            //关闭连接
    ...
    public void auth(Channel channel, MqttMessage mqttAuthMessage) {
        ...
        if (mqttVariableHeader.reasonCode() == com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_CONTINUE_REASON_CODE) {
            ...
            if (mqttAuthenticator != null) {
                ...
                if (mqttAuthInstruct != null) {
                    ...
                    if (nextInstruct == MqttAuthInstruct.Instruct.AUTH_FAIL || nextInstruct == MqttAuthInstruct.Instruct.AUTH_CONTINUE) {
                        authenticateReasonCode = com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_CONTINUE_REASON_CODE;
                    } else if (nextInstruct == MqttAuthInstruct.Instruct.RE_AUTH) {
                        //重新认证
                        authenticateReasonCode = com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_RE_AUTH_REASON_CODE;
                    } else {
    ...
    public void sendDisconnect(Channel channel, MqttFuture mqttFuture, MqttDisconnectMsg mqttDisconnectMsg) {
        ...
        channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.DISCONNECT_REASON_CODE_ATTRIBUTE_KEY).set(reasonCode);
    ...
    public void disconnect(Channel channel, MqttMessage mqttMessage) {
        ...
        MqttAuthState mqttAuthState = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.AUTH_STATE_ATTRIBUTE_KEY).get();
    ...
    private MqttMsg getQos2SendMsg(Channel channel, int msgId) {
        ...
        if (mqttConnectParameter.isCleanSession()) {
            //清理会话直接从Channel中获取
            Object msg = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.SEND_MSG_MAP_ATTRIBUTE_KEY).get().get(msgId);
    ...
    private void putReceiveQos2Msg(Channel channel, MqttMsg mqttMsg) {
        if (mqttConnectParameter.isCleanSession()) {
            channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.RECEIVE_MSG_MAP_ATTRIBUTE_KEY).get().put(mqttMsg.getMsgId(), mqttMsg);
        } else {
    ...
    private MqttMsg removeHighQosMsg(Channel channel, int msgId, MqttMsgDirection msgDirection) {
        ...
        if (mqttConnectParameter.isCleanSession()) {
            if (msgDirection == MqttMsgDirection.SEND) {
                Object msg = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.SEND_MSG_MAP_ATTRIBUTE_KEY).get().remove(msgId);
                if (msg instanceof MqttMsg) {
                    mqttMsg = (MqttMsg) msg;
                }
            } else {
                mqttMsg = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.RECEIVE_MSG_MAP_ATTRIBUTE_KEY).get().remove(msgId);
            }
        } else {
    ...
    private void updateQos2MsgState(Channel channel, int msgId, MqttMsgState msgState, MqttMsgDirection msgDirection) {
        if (mqttConnectParameter.isCleanSession()) {
            ...
            if (msgDirection == MqttMsgDirection.SEND) {
                ...
                MqttMsg mqttMsg = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.RECEIVE_MSG_MAP_ATTRIBUTE_KEY).get().get(msgId);
                if (mqttMsg != null) {
                    mqttMsg.setMsgState(msgState);
                }
            }
    ...
    private String topicAliasHandle(Channel channel, MqttMsg mqttMsg) {
        ...
        if (mqttProperties != null && mqttConnectParameter.getMqttVersion() == com.yh.imserver.netty.mqtt.client.constant.MqttVersion.MQTT_5_0_0) {
            ...
            Attribute<Map<String, Integer>> topicAliasMapAttribute = channel.attr(com.yh.imserver.netty.mqtt.client.constant.MqttConstant.TOPIC_ALIAS_MAP_ATTRIBUTE_KEY);
                ...
                synchronized (topic.intern()) {
                    ...
                    if (topicAlias != null) {
                        if (topicAlias.equals(nowTopicAlias)) {
                            //别名一致，返回空字符作为主题
                            topic = com.yh.imserver.netty.mqtt.client.constant.MqttConstant.EMPTY_STR;
                        } else {
                            //别名不一致时，使用现在的替换
